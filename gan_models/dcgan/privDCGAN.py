...
def train_privGAN(genS, discS, private_disc, opt_gen, opt_disc, opt_private_disc, t, criterion, loss_fn):
    ...
    if args.save_models:
        ...
        for i in range(args.N_splits):
            torch.save(genS[i].state_dict(), os.path.join(dirname, f"gen{i}.pt") )
            torch.save(discS[i].state_dict(), os.path.join(dirname, f"disc{i}.pt") )
        torch.save(private_disc.state_dict(), os.path.join(dirname, "private_disc.pt"))
